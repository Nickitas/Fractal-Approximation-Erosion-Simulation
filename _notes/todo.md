...
**ТЗ:** Добавить функцию LoadCoastlineData() []LatLon, которая hardcoded массив из 10-15 реальных точек берега Черного моря (например, от Википедии: Одесса {46.48, 30.73}, Севастополь {44.62, 33.53} и т.д. — использовать approx. значения). Заменить случайные точки на эти, вычислить длину в км (уч. масштаб: approx. 1 единица = 1 км).

### ✅ 5: Простая фрактальная итерация (Кривая Коха на сегменте)

**Улучшение:** Ввести фрактальную геометрию для создания неровностей.  
**ТЗ:** Создать функцию KochCurve(points []LatLon, iterations int) []LatLon, которая для каждого сегмента добавляет треугольник (стандартная итерация Коха). Применить 1 итерацию к полилинии из части 4, вычислить новую длину (она вырастет).

### ✅ 6: Множественные итерации фрактала
...
### ✅ 7: Вычисление фрактальной размерности

**Улучшение:** Добавить математический анализ (box-counting метод approx.).  
**ТЗ:** Создать функцию FractalDimension(points []LatLon) float64, которая approx. вычисляет размерность (логарифм числа коробок / логарифм масштаба). Применить после фрактальной генерации, сравните с теоретическим для Коха (~1.26).
Дополнительно: визуализация svg и сайт

### ✅ 8: Чтение данных из JSON-файла

**Улучшение:** Сделать данные динамичными, загрузка из файла.  
**ТЗ:** Создать JSON-файл с массивом точек (реальные координаты Черного моря). Добавить функцию LoadFromJSON(filename string) []LatLon с использованием encoding/json и os. Заменить hardcoded данные, обработайте ошибки.

### ✅ 9: Загрузка реальных данных через HTTP

**Улучшение:** Получить актуальные данные онлайн (например, от API или geojson).  
**ТЗ:** Добавить функцию FetchCoastlineData(url string) []LatLon, используя net/http для GET-запроса к открытому geojson (например, hypothetical "https://example.com/blacksea.json"). Парсить JSON, извлеките координаты. Обновить загрузку, fallback на локальный файл.

- Разделить в UX “реальные расчёты” и “синтетические демонстрации”, чтобы пользователь не путал их.
- Добавить кэш скачанного GeoJSON с --refresh, чтобы не ходить в сеть на каждый запуск.
- Добавить упрощение геометрии для SVG и тяжёлых команд, потому что 9635 точек уже заметно нагружают пайплайн.
- Сделать отдельную команду вроде source или fetch, которая покажет метаданные набора и сохранит snapshot локально.
- Обновить README: явно написать, что koch и dimension используют реальные данные только как базовую полилинию.

### ⬜️ 10: Базовая визуализация в SVG

**Улучшение:** Добавить вывод графики для визуального представления.  
**ТЗ:** Использовать библиотеку (go get github.com/ajstarks/svgo), создать функцию DrawSVG(points []LatLon, filename string) для рисования полилинии в SVG-файл. Вызывать после фрактальной генерации.

### ⬜️ 11: Симуляция эрозии (случайное смещение точек)

**Улучшение:** Ввести динамику: модель эрозии как случайные сдвиги.  
**ТЗ:** Добавить функцию Erode(points []LatLon, strength float64) []LatLon, которая для каждой точки добавляет случайный вектор (gauss распределение). Применить после фрактала, пересчитайте длину и размерность, сгенерируйте новый SVG.

### ⬜️ 12: Множественные шаги симуляции эрозии

**Улучшение:** Сделать симуляцию временной (несколько шагов).  
**ТЗ:** Создать функцию SimulateErosion(points []LatLon, steps int, strength float64) [][]LatLon, возвращающую слайс состояний на каждом шаге. В main выведите метрики для каждого шага, сгенерировать серию SVG-файлов.

### ⬜️ 13: Concurrency для ускорения симуляций

**Улучшение:** Распараллеливать вычисления для больших наборов точек.  
